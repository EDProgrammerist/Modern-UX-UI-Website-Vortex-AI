...
import { VortexAI } from "@vortex /ai";

function App() {
  const [code, setCode] = useState("");

  const handleAICompletion = async () => {
    const suggestion = await VortexAI.complete(code);
    setCode(suggestion);
  };
...
import { Vortex AI } from "@Vortex/ai";
...
export default function Hero() {
  ...
  const handleAISuggestion = async () => {
    const suggestion = await Vortex AI.suggest("hero component");
    return suggestion;
  };
...
export default function Navbar() {
  ...
  const handleSearch = async () => {
    const results = await VortexAI.search(searchQuery);
    return results;
  };
